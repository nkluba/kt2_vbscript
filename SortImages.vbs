...
Dim fso, startFolder, targetFolder
...
Sub SortFiles(folderPath, targetPath)
    ...
    For Each file In folder.Files
        If LCase(fso.GetExtensionName(file)) = "jpg" Or LCase(fso.GetExtensionName(file)) = "jpeg" Or LCase(fso.GetExtensionName(file)) = "png" Then
            ...
            If Not fso.FolderExists(targetPath & "\" & Year(file.DateLastModified)) Then fso.CreateFolder(targetPath & "\" & Year(file.DateLastModified))
            If Not fso.FolderExists(targetDateFolder) Then fso.CreateFolder(targetDateFolder)
